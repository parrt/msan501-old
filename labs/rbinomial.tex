...
\step Now that we can know how to get a binomial random variable, we can examine the binomial distribution.  All we have to do is grab a vector of, say, $N$ binomial random values and the plot a histogram.  The density function at $k$ is just how many successes out of $N$ there were ($k/N$, actually).
...
\begin{pyverbatim}
X = [binomial(N,P) for t in range(N)]
\end{pyverbatim}
